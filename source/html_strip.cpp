...
static bool isSkipContentTag(const char* tag, int len) {
    ...
    for (int i = 0; SKIP[i]; ++i) {
        ...
        if (sl == len && strncasecmp(tag, SKIP[i], len) == 0)
            return true;
    }
    return false;
}
...
static bool isBlockTag(const char* tag, int len) {
    static const char* BLOCK[] = {"p",  "div", "br", "li", "h1",         "h2",  "h3",
                                  "h4", "h5",  "h6", "tr", "blockquote", "pre", nullptr};
    for (int i = 0; BLOCK[i]; ++i) {
        int bl = (int)strlen(BLOCK[i]);
        if (bl == len && strncasecmp(tag, BLOCK[i], len) == 0)
            return true;
    }
    return false;
}
            ...
            if (!isClose && isSkipContentTag(tagStart, nameLen)) {
                ...
                for (const char* q = p; q + copyLen + 2 < end; ++q) {
                    ...
                    for (int i = 0; closeTag[i] && match; ++i)
                        if (tolower((unsigned char)q[i]) != closeTag[i])
                            match = false;
                    if (match) {
                        found = q;
                        break;
