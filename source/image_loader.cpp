 ...
 */
static void bilinearResize(const uint8_t* src, int sw, int sh, uint8_t* dst, int dw, int dh) {
        ...
        for (int dx = 0; dx < dw; ++dx) {
            ...
            for (int c = 0; c < 4; ++c) {
                float v = (float)p00[c] * (1.0f - wx) * (1.0f - wy) +
                          (float)p10[c] * wx * (1.0f - wy) + (float)p01[c] * (1.0f - wx) * wy +
                          (float)p11[c] * wx * wy;
                int iv = (int)(v + 0.5f);
                if (iv < 0)
                    iv = 0;
 ...
 * larger dimension is at most MAX_DIM while preserving aspect ratio (minimum dimension 1), and
 * writes pixels into `out.rgba` with `out.imgW`/`out.imgH` set accordingly. On failure `out.failed`
 * is set.
 *
 * @param url Source URL of the image to download.
 * @param out Output container that will be populated with the decoded/resized image or marked
 * failed.
 ...
 */
static void processOne(const std::string& url, DecodedImage& out, XferInfoFn xferFn, void* xferUd,
                       int maxDim) {
    std::string err;
    ...
    int w = 0, h = 0, ch = 0;
    uint8_t* px = stbi_load_from_memory(bin.data(), (int)bin.size(), &w, &h, &ch, 4);
    if (!px || w <= 0 || h <= 0) {
        if (px)
            stbi_image_free(px);
        out.failed = true;
 ...
 * @param dltotal  Total number of bytes expected for this URL; if <= 0 the function will
 * approximate the total using `MAX_BYTES`.
 */
