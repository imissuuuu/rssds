...
static u32 computeRepeat(u32 kDown, u32 kHeld, KeyRepeatState& rs, u32 delayMs, u32 intervalMs) {
    u64 now = osGetTime();
    u32 result = 0;
...
int main() {
    ...
    std::vector<FeedConfig> configs = loadFeedConfig("sdmc:/3ds/rssreader/feeds.json");
    ...
    while (aptMainLoop()) {
        ...
        if (state.currentScreen == Screen::ArticleView) {
            kRepeat =
                computeRepeat(kDown, kHeld, repeatState, (u32)state.settings.scrollRepeatDelayMs,
            ...
            if (state.articleLoader.poll(result, errMsg)) {
                if (!result.body.empty()) {
                    if (state.pendingFetchFeed == -2) {
                        ...
                        if (state.pendingFetchFullArticle)
                            state.bookmarkTempArticle.fullFetched = true;
                        state.viewingBookmark = true;
                    } else {
                        auto& art =
                            state.feeds[state.pendingFetchFeed].articles[state.pendingFetchArticle];
