...
static bool tagEq(lxb_dom_node_t* n, const char* name) {
    ...
    const lxb_char_t* tag = lxb_dom_element_local_name(
        lxb_dom_interface_element(n), &len);
    if (!tag) return false;
    for (size_t i = 0; i < len; ++i) {
        if (!name[i]) return false;
        if (tag[i] != (lxb_char_t)name[i]) return false;
    }
    return name[len] == '\0';
}
...
static bool tagIn(lxb_dom_node_t* n, const char** tags) {
    for (int i = 0; tags[i]; ++i)
        if (tagEq(n, tags[i])) return true;
    return false;
}

static const lxb_char_t* getAttr(lxb_dom_node_t* n,
                                  const char* name, size_t* len_out) {
    if (!n || n->type != LXB_DOM_NODE_TYPE_ELEMENT) return nullptr;
...
static bool attrContainsAny(const lxb_char_t* val, size_t len,
                             const char** kw) {
            ...
            for (int ki = 0; kw[ki]; ++ki) {
                ...
                for (size_t j = 0; j + klen <= tlen; ++j) {
                    ...
                    for (size_t k = 0; k < klen && match; ++k)
                        match = (tolower((unsigned char)val[tok+j+k])
                                 == (unsigned char)kw[ki][k]);
...
static void getTextInfo(lxb_dom_node_t* node, size_t& total,
                        size_t& link, size_t& commas) {
...
static double classBias(lxb_dom_node_t* node) {
    static const char* POSITIVE[] = {
        ...
        "post","text","blog","story", nullptr
    };
    static const char* NEGATIVE[] = {
        "hidden","hid","ad-","ads","agegate","popup","extra","print", nullptr
    };
    size_t len = 0;
    const lxb_char_t* cls = getAttr(node, "class", &len);
    if (attrContainsAny(cls, len, POSITIVE))  return  25.0;
    if (attrContainsAny(cls, len, NEGATIVE))  return -25.0;
    const lxb_char_t* id = getAttr(node, "id", &len);
    if (attrContainsAny(id, len, POSITIVE))   return  25.0;
    if (attrContainsAny(id, len, NEGATIVE))   return -25.0;
    return 0.0;
}

// タグ別 base score
static double tagBaseScore(lxb_dom_node_t* node) {
    if (tagEq(node,"div"))        return 5.0;
    if (tagEq(node,"blockquote")) return 3.0;
    if (tagEq(node,"pre"))        return 3.0;
    if (tagEq(node,"td"))         return 3.0;
    if (tagEq(node,"article"))    return 10.0;
    if (tagEq(node,"section"))    return 5.0;
    if (tagEq(node,"li"))         return -3.0;
    if (tagEq(node,"address"))    return -3.0;
    return 0.0;
}

// Candidates 配列から node を検索し見つかればスコアを加算、なければ追加
static void addScore(std::vector<Candidate>& cands,
                     lxb_dom_node_t* node, double score) {
    if (!node) return;
    for (auto& c : cands) {
        if (c.node == node) { c.score += score; return; }
    }
...
static lexbor_action_t score_cb(lxb_dom_node_t* node, void* ctx) {
    ...
    // テキスト系ブロック要素でスコア計算
    if (node->type != LXB_DOM_NODE_TYPE_ELEMENT) return LEXBOR_ACTION_OK;

    static const char* SCORE_TAGS[] = { "p","pre","td", nullptr };
    if (!tagIn(node, SCORE_TAGS)) return LEXBOR_ACTION_OK;

    size_t total = 0, link = 0, commas = 0;
    getTextInfo(node, total, link, commas);
    if (total < 25) return LEXBOR_ACTION_OK; // 短すぎるノードは無視

    double lk_density = (double)link / (double)total;
    if (lk_density > 0.5) return LEXBOR_ACTION_OK; // リンクが半分超は無視

    // スコア = (文字数/100 の min 3) + (コンマ数) + 1
    double score = 1.0
                 + (double)commas
                 + std::min((double)(total / 100), 3.0);
    ...
    if (parent) addScore(*sc->cands, parent, score + tagBaseScore(parent) + classBias(parent));
    if (grand)  addScore(*sc->cands, grand,  score / 2.0);
    if (great)  addScore(*sc->cands, great,  score / 3.0);
...
static lxb_dom_node_t* findTopCandidate(lxb_dom_node_t* body,
    ...
    struct PriCtx { lxb_dom_node_t* found; };
    PriCtx pc = {nullptr};
    lxb_dom_node_simple_walk(body,
        [](lxb_dom_node_t* n, void* ctx) -> lexbor_action_t {
            auto* pctx = static_cast<PriCtx*>(ctx);
            static const char* PTAGS[] = { "article","main", nullptr };
            if (tagIn(n, PTAGS)) { pctx->found = n; return LEXBOR_ACTION_STOP; }
    ...
    for (auto& c : cands) {
        ...
        if (adj > topScore) { topScore = adj; top = c.node; }
    }
    return top;
}
...
static std::vector<lxb_dom_node_t*> collectSiblings(
        lxb_dom_node_t* top,
    ...
    if (!parent) { result.push_back(top); return result; }
    ...
    while (sibling) {
        if (sibling == top) {
            ...
            if (tagEq(sibling, "p")) {
                ...
                if (total >= 80 && ld < 0.25) {
                    bool already = false;
                    for (auto* r : result) if (r == sibling) { already = true; break; }
                    if (!already) result.push_back(sibling);
                }
            }
        }
        sibling = lxb_dom_node_next(sibling);
    }
    if (result.empty()) result.push_back(top);
    return result;
}
...
static void logEntry(const char* url, const char* status,
                     const char* winner, double score,
...
std::string extractContent(const std::string& html,
    ...
    if (!top) {
        ...
        // winner タグ名をログ用に記録
        if      (tagEq(top, "article")) logWinner = "article";
        else if (tagEq(top, "main"))    logWinner = "main";
        else if (tagEq(top, "div"))     logWinner = "div";
        else if (tagEq(top, "section")) logWinner = "section";
        else                            logWinner = "other";
        logScore = topScore;
    }
