 ...
 * @param url Source identifier to be processed; callers are responsible for avoiding duplicate
 * submissions.
 */

/**
 * Remove all jobs that have not yet started. If a job is currently in flight, wait for it to
 * finish.
 */
 ...
 * @returns A value in the range 0.0 to 1.0 representing completion percentage; returns 0.0 if the
...
/**
 * Main loop executed by the worker thread; consumes jobs, produces DecodedImage results, and
 * updates progress.
 */
